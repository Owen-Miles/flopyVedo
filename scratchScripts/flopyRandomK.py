...
print("number of cells: ", ncpl)
print("number of vertex pairs: ", nvert)
...
# properties
k = 0.0001
mean, sigma = -4, 1 # mean and standard deviation of the random k field
k = 10**np.random.normal(mean, sigma, ncpl)
...
# npf = flopy.mf6.ModflowGwfnpf(gwf, k=[1,2,3,4,4,3,2,1])
...
klog= np.log10(k)
img = tri.plot(ax=ax, a=klog, cmap='Spectral')
mplt.colorbar(img, fraction=0.02)
mplt.show()
